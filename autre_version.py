...
def terrain_aléatoire():
    ...
    ### range la couleur dans une liste 2D [ligne] [colonne] ###
    for i in range(20):
        for j in range(40):
            tableau.append([])
...
def Propage_feu():
    """Traite la case de colonne i et de ligne j et retourne la nouvelle valeur dans le tableau"""
   for i in range(1, NB_COL-1):
        for j in range(1, NB_LINE-1):
            if tableau[i][j]=="yellow2":
                if tableau[i-1][j]=="firebrick3" or tableau[i+1][j]=="firebrick3" or tableau[i][j-1]=="firebrick3" or tableau[i][j+1]=="firebrick3":
                   canvas.create_rectangle(Coord_Case(i, j), fill = "firebrick3")
                   tableau [i] [j] = "firebrick3"
